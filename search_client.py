...
class SearchClient:
    ...
    saturer les moteurs et le LLM de synthesis.

    recent est laissé à False par défaut, y compris pour l'enrichissement du
    bulletin : le mode actualité de agent-web-search interroge la presse et
    remonte donc les mêmes quotidiens que les flux RSS déjà crawlés (mesuré à
    5 sources sur 8 sur un sujet chaud). Le mode intemporel apporte au
    contraire des sources que le crawl n'a pas — institutionnels, agences,
    presse étrangère —, ce qu'on attend d'un enrichissement.
    """
    ...
    async def search(self, query: str, recent: bool = False,
        ...
        async with self._sem:
            result = await self._nexus.request(
                ...
                {"query": query, "recent": recent, "n_results": 10,
    ...
    async def search_many(self, queries: list[str], recent: bool = False) -> list[dict]:
        async def _one(query: str, i: int) -> dict:
            logger.info(f"    recherche [{i+1}/{len(queries)}] → {query[:70]!r}")
            r = await self.search(query, recent)
